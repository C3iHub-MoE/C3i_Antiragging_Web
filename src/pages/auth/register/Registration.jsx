...
const UserRegister = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!isCollegeValid(collegeName)) {
      setCollegeError(
        "Collage must be at least 5 characters and contain only letters and numbers."
      );
      return;
    }
    if (!isStateValid(selectedState)) {
      setStateError(
        "State must be at least 5 characters and contain only letters"
      );
      return;
    }
    ...
    if (!phonePattern.test(phoneNumber)) {
      setPhoneError(
        "Phone number must be 10 digits or in the format +91 1234567890."
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        "http://172.29.27.254:8000/api/students/register/",
        formData
      );
      ...
      <div className={styles.registerBox}>
        ...
        <form onSubmit={handleRegister} className={styles.form}>
          ...
          <div className={styles.inputGroup}>
            ...
            />
            {districtError && (
              <span className={styles.error}>{districtError}</span>
            )}
          </div>
          ...
          {role === "student" && (
            <>
              <div className={styles.inputGroup}>
                ...
                {collegeError && (
                  <span className={styles.error}>{collegeError}</span>
                )}
              </div>
              <div className={styles.inputGroup}>
                <label className={styles.label}>College Name</label>
