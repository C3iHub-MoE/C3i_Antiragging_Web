...
const SosBarChart = () => {
  ...
  useEffect(() => {
    if (sosData && sosData.length > 0) {
      ...
      const totalIncidents = labels.map(
        (state) => stateWiseData[state].totalIncidents
      );
      const avgResponseTime = labels.map((state) =>
        stateWiseData[state].count > 0
          ? (
              stateWiseData[state].totalResponseTime /
              stateWiseData[state].count
            ).toFixed(1)
          : 0
      );
